document.addEventListener("DOMContentLoaded", () => {
  ...
  const yearlySavingsSpan = document.getElementById("yearlySavings");
  ...
  const baseSavingsRate = 0.65; // assume AI replaces 65% of the cost

  function formatCurrency(amount) {
    if (amount >= 100000) {
      const inLakhs = (amount / 100000).toFixed(2);
      monthlySavingsSpan.setAttribute("aria-label", `${inLakhs} lakh rupees`);
      return `₹${inLakhs}L`;
    }
    monthlySavingsSpan.setAttribute(
      "aria-label",
      `${amount.toLocaleString()} rupees`
    );
    return `₹${amount.toLocaleString()}`;
  }

  function estimateSavings(monthlySpend, hoursPerWeek, taskType) {
    let aiCost = Math.max(3000, 200 * hoursPerWeek); // base AI infra cost

    // Adjust AI cost based on task type
    switch (taskType) {
      case "customer_support":
        aiCost *= 1.2; // Higher cost for customer support automation
        break;
      case "lead_generation":
        aiCost *= 1.3; // Higher cost for lead gen automation
        break;
      default:
        break;
    }

    const estimatedSavings = (monthlySpend - aiCost) * baseSavingsRate;
    return Math.max(estimatedSavings, 0);
  }
  ...
  monthlySpendInput.addEventListener("input", () =>
    validateInput(monthlySpendInput)
  );
  weeklyHoursInput.addEventListener("input", () =>
    validateInput(weeklyHoursInput)
  );
  ...
  calculatorForm.addEventListener("submit", (e) => {
    ...
    const monthlySavings = estimateSavings(monthlySpend, weeklyHours, taskType);
    const yearlySavings = monthlySavings * 12;

    monthlySavingsSpan.textContent = formatCurrency(monthlySavings);
    yearlySavingsSpan.textContent = formatCurrency(yearlySavings);

    savingsResult.style.display = "block";
    savingsResult.scrollIntoView({ behavior: "smooth" });
  });
...
const leCardContentsLand = [
  {
    ...
    contentDescription1:
      "Identify hidden stakeholders and buying groups to engage the right decision-makers.",
    contentHeading2: "Scale Smarter",
    contentDescription2:
      "Enhance rep productivity by ensuring every interaction is with a high-intent buyer.",
  },
  {
    title: "RevOps",
    leftSubheading: "Drive new revenue with precision.",
    ...
    contentDescription1:
      "Surface hidden stakeholders and fragmented buying centers for accurate forecasting.",
    contentHeading2: "AI-Driven Targeting",
    contentDescription2:
      "Pinpoint high-value enterprise accounts and map decision-makers for predictable growth.",
  },
  {
    ...
    contentDescription1:
      "Cut deal research time by 80% and accelerate sales cycles.",
    contentHeading2: "Multi-thread with Confidence ",
    contentDescription2:
      "Uncover decision-makers, buying signals, and competitive gaps to win more deals.",
  },
];
...
const leCardContentsExpand = [
  {
    ...
    contentDescription1:
      "Predict which customers are ready to buy more using AI-driven engagement signals",
    contentHeading2: "Expand Across Entities ",
    contentDescription2:
      "Identify and engage buying centers across subsidiaries and regions for multi-entity expansion.",
  },
  {
    title: "RevOps",
    leftSubheading: "Maximize growth within existing accounts.",
    ...
    contentDescription1:
      "Automate account prioritization, renewal risk tracking, and outreach strategies.",
    contentHeading2: "Unlock Expansion Revenue",
    contentDescription2:
      "Surface hidden upsell and cross-sell opportunities using product usage trends and buyer intent.",
  },
  {
    ...
    contentDescription1:
      "Identify all buying entities, hidden champions, and budget owners upfront.",
    contentHeading2: "Close Expansion Deals Faster",
    contentDescription2:
      "Walk into every meeting with pre-warmed insights on decision-makers and team needs.",
  },
];
...
const leContentDescription1 = document.getElementById(
  "le-content-description-1"
);
const leContentHeading2 = document.getElementById("le-content-heading-2");
const leContentDescription2 = document.getElementById(
  "le-content-description-2"
);
...
function leUpdateContent(tab, cardIndex) {
  ...
  const content =
    tab === "land"
      ? leCardContentsLand[cardIndex]
      : leCardContentsExpand[cardIndex];
...
function updateAccordionContent(contentArray, tab) {
  const accordions = document.querySelectorAll(
    `.le-accordion-item[data-tab="${tab}"]`
  );
  ...
  accordions.forEach((accordion, index) => {
    ...
    const section1Title = accordion.querySelector(
      ".le-accordion-section:first-child .le-accordion-section-title"
    );
    const section1Text = accordion.querySelector(
      ".le-accordion-section:first-child .le-accordion-section-text"
    );
    const section2Title = accordion.querySelector(
      ".le-accordion-section:last-child .le-accordion-section-title"
    );
    const section2Text = accordion.querySelector(
      ".le-accordion-section:last-child .le-accordion-section-text"
    );
